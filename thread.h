...
public:
	...
	/*
	 ...
	 追記 : 一つのThread変数につき一つまでしかスレッドは作成できません。
			...
			作成した場合、失敗します。
	*/
	bool CreateThread(ThreadData* tData, _beginthreadex_proc_type ThreadProc, bool bOnDest, Thread **lpThread);
	...
	*/
	...
	/*
	 ...
	 追記 : この関数を使用すると、現在スレッド内で行っている処理を強制的に中断しスレッドを終了します。
			また、スレッド内で確保したメモリ領域は解放されません。その為、メモリリークの危険性があります。
...
public:
	// コンストラクタ
	DevData() : bSafe(true), nCountEnterSection(0) { InitializeCriticalSection(&cs); }

	// デストラクタ
	~DevData() { bSafe = false; nCountEnterSection = 0; DeleteCriticalSection(&cs); }

	// クリティカルセクション獲得開始
	void LockDevData(void)
	{
		if (bSafe)
		{
